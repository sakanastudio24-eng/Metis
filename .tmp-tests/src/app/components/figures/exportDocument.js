    ...
    return {
        ...
        sections: [
            ...
            },
            {
                id: "issues",
                title: "Top Issues",
                lines: viewModel.topIssues.length > 0
                    ? viewModel.topIssues.map((issue) => `${issue.title} · ${issue.severityLabel} · ${issue.detail}`)
                    : ["No major cost issues surfaced on this route."]
            },
            {
                id: "stack",
                title: "Known Stack",
                lines: viewModel.stackGroups.length > 0
                    ? viewModel.stackGroups.map((group) => `${group.label}: ${group.items.map((item) => item.label).join(", ")}`)
                    : ["No strong cost-relevant stack groups were resolved."]
            },
            {
                id: "recommendations",
                title: "Recommendations",
                lines: viewModel.fixRecommendationCards.length > 0
...
function buildExportOutlineText(document) {
    return [
        document.title,
