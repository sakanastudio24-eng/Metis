...
exports.buildMetisDesignViewModel = buildMetisDesignViewModel;
/**
 * liveAdapter.ts
...
function scoreToRiskTone(score) {
    switch (score.label) {
        case "high risk":
            return {
                label: "High Risk",
...
function deriveMonthlyWaste(snapshot, answers) {
    ...
    const baseWaste = metrics.totalEncodedBodySize / 1_000_000 * 1.1 +
        ...
        aiWeight(answers);
    const visitFactor = answers.monthlyVisits === "under1k"
        ? 0.4
...
function issueToDesignIssue(issue) {
    return {
        id: issue.id,
        title: issue.title,
        detail: issue.detail,
        severityLabel: issue.severity === "high"
            ? "critical"
...
function buildSummaryPills(issues) {
    const counts = {
    ...
    return [
        counts.critical > 0
            ? { label: `${counts.critical} Critical`, tone: "critical" }
            : null,
        counts.moderate > 0
            ? { label: `${counts.moderate} Moderate`, tone: "moderate" }
            : null,
        counts.low > 0 ? { label: `${counts.low} Low`, tone: "low" } : null
    ].filter((value) => value !== null);
}
function buildCostRows(score, snapshot, answers) {
    ...
    return [
        ...
        {
            label: answers.aiUsage && answers.aiUsage !== "no"
                ? "AI API Usage"
                : "Third-Party / APIs",
...
function buildQuestionState(report, requiredCount) {
    if (!report) {
        return {
            answeredCount: 0,
            requiredCount,
...
function buildScaleSimulationRows(monthlyWaste) {
    const cases = [
