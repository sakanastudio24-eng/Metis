...
function meetsDuplicateThreshold(snapshot, threshold) {
    return (snapshot.metrics.duplicateRequestCount >= threshold.duplicateRequestCount ||
        snapshot.metrics.duplicateEndpointCount >= threshold.duplicateEndpointCount);
}
function meetsLargeImageThreshold(snapshot, threshold) {
    return (snapshot.metrics.meaningfulImageBytes >= threshold.meaningfulImageBytes ||
        snapshot.metrics.meaningfulImageCount >= threshold.meaningfulImageCount);
}
function isScoredHostingVendor(vendor) {
    return !["aws", "aws-s3", "aws-api-gateway", "cloudfront"].includes(vendor.id);
}
function detectIssues(snapshot, answers = {}) {
    const { metrics } = snapshot;
    const issues = [];
    const moneyStack = (0, stack_1.detectMoneyStack)(snapshot, answers);
    ...
    if (metrics.requestCount >= config_1.DETECTION_THRESHOLDS.requestCount.high) {
        issues.push({
            ...
            threshold: {
                requestCount: config_1.DETECTION_THRESHOLDS.requestCount.high
            }
        });
    }
    else if (metrics.requestCount >= config_1.DETECTION_THRESHOLDS.requestCount.medium) {
        issues.push({
            id: "high-request-count",
            ...
            threshold: {
                requestCount: config_1.DETECTION_THRESHOLDS.requestCount.medium
            }
        });
    }
    else if (metrics.requestCount >= config_1.DETECTION_THRESHOLDS.requestCount.low) {
        issues.push({
            id: "high-request-count",
            ...
            threshold: {
                requestCount: config_1.DETECTION_THRESHOLDS.requestCount.low
            }
        });
    }
    if (meetsDuplicateThreshold(snapshot, config_1.DETECTION_THRESHOLDS.duplicateRequests.high)) {
        issues.push({
            ...
            threshold: config_1.DETECTION_THRESHOLDS.duplicateRequests.high
        });
    }
    else if (meetsDuplicateThreshold(snapshot, config_1.DETECTION_THRESHOLDS.duplicateRequests.medium)) {
        issues.push({
            id: "duplicate-requests",
            ...
            threshold: config_1.DETECTION_THRESHOLDS.duplicateRequests.medium
        });
    }
    else if (meetsDuplicateThreshold(snapshot, config_1.DETECTION_THRESHOLDS.duplicateRequests.low)) {
        issues.push({
            id: "duplicate-requests",
            ...
            threshold: config_1.DETECTION_THRESHOLDS.duplicateRequests.low
        });
    }
    if (metrics.totalEncodedBodySize >= config_1.DETECTION_THRESHOLDS.pageWeight.high) {
        issues.push({
            ...
            threshold: {
                totalEncodedBodySize: config_1.DETECTION_THRESHOLDS.pageWeight.high
            }
        });
    }
    else if (metrics.totalEncodedBodySize >= config_1.DETECTION_THRESHOLDS.pageWeight.medium) {
        issues.push({
            id: "heavy-page-weight",
            ...
            threshold: {
                totalEncodedBodySize: config_1.DETECTION_THRESHOLDS.pageWeight.medium
            }
        });
    }
    else if (metrics.totalEncodedBodySize >= config_1.DETECTION_THRESHOLDS.pageWeight.low) {
        issues.push({
            id: "heavy-page-weight",
            ...
            threshold: {
                totalEncodedBodySize: config_1.DETECTION_THRESHOLDS.pageWeight.low
            }
        });
    }
    if (meetsLargeImageThreshold(snapshot, config_1.DETECTION_THRESHOLDS.largeImages.high)) {
        issues.push({
            id: "large-images",
            ...
            threshold: config_1.DETECTION_THRESHOLDS.largeImages.high
        });
    }
    else if (meetsLargeImageThreshold(snapshot, config_1.DETECTION_THRESHOLDS.largeImages.medium)) {
        issues.push({
            id: "large-images",
            ...
            threshold: config_1.DETECTION_THRESHOLDS.largeImages.medium
        });
    }
    else if (meetsLargeImageThreshold(snapshot, config_1.DETECTION_THRESHOLDS.largeImages.low)) {
        issues.push({
            id: "large-images",
            ...
            threshold: config_1.DETECTION_THRESHOLDS.largeImages.low
        });
    }
    if (metrics.thirdPartyDomainCount >= config_1.DETECTION_THRESHOLDS.thirdPartySprawl.high) {
        issues.push({
            ...
            threshold: {
                thirdPartyDomainCount: config_1.DETECTION_THRESHOLDS.thirdPartySprawl.high
            }
        });
    }
    else if (metrics.thirdPartyDomainCount >= config_1.DETECTION_THRESHOLDS.thirdPartySprawl.medium) {
        issues.push({
            id: "third-party-sprawl",
            ...
            threshold: {
                thirdPartyDomainCount: config_1.DETECTION_THRESHOLDS.thirdPartySprawl.medium
            }
        });
    }
    else if (metrics.thirdPartyDomainCount >= config_1.DETECTION_THRESHOLDS.thirdPartySprawl.low) {
        issues.push({
            id: "third-party-sprawl",
            ...
            threshold: {
                thirdPartyDomainCount: config_1.DETECTION_THRESHOLDS.thirdPartySprawl.low
            }
        });
    }
    ...
    const analyticsVendors = moneyStack.groups.find((group) => group.id === "analyticsAdsRum")?.vendors ?? [];
    if (analyticsVendors.length >= config_1.DETECTION_THRESHOLDS.analyticsAdsRumSurface.high) {
        issues.push({
            ...
            threshold: {
                analyticsVendorCount: config_1.DETECTION_THRESHOLDS.analyticsAdsRumSurface.high
            }
        });
    }
    else if (analyticsVendors.length >= config_1.DETECTION_THRESHOLDS.analyticsAdsRumSurface.medium) {
        issues.push({
            id: "analytics-ads-rum-surface",
            ...
            threshold: {
                analyticsVendorCount: config_1.DETECTION_THRESHOLDS.analyticsAdsRumSurface.medium
            }
        });
    }
    else if (analyticsVendors.length >= config_1.DETECTION_THRESHOLDS.analyticsAdsRumSurface.low) {
        issues.push({
            id: "analytics-ads-rum-surface",
            ...
            threshold: {
                analyticsVendorCount: config_1.DETECTION_THRESHOLDS.analyticsAdsRumSurface.low
            }
        });
    }
    ...
    if (aiVendors.length > 0 && (metrics.apiRequestCount > 0 || aiUsageHint)) {
        const highAi = metrics.apiRequestCount >= config_1.DETECTION_THRESHOLDS.aiSpendSurface.highApiRequestCount ||
            metrics.requestCount >= config_1.DETECTION_THRESHOLDS.aiSpendSurface.highRequestCount ||
            answers.aiUsage === "yesOften";
        const mediumAi = metrics.apiRequestCount >= config_1.DETECTION_THRESHOLDS.aiSpendSurface.mediumApiRequestCount ||
            metrics.requestCount >= config_1.DETECTION_THRESHOLDS.aiSpendSurface.mediumRequestCount;
        issues.push({
            id: "ai-spend-surface",
            title: highAi
            ...
            threshold: {
                apiRequestCount: highAi
                    ? config_1.DETECTION_THRESHOLDS.aiSpendSurface.highApiRequestCount
                    : mediumAi
                        ? config_1.DETECTION_THRESHOLDS.aiSpendSurface.mediumApiRequestCount
                        : 1
            }
        });
    }
    ...
    if (scoredHostingVendors.length > 0) {
        const highHosting = scoredHostingVendors.length >= 2 &&
            (metrics.totalEncodedBodySize >= config_1.DETECTION_THRESHOLDS.hostingCdnSpendSurface.highTransferBytes ||
                metrics.requestCount >= config_1.DETECTION_THRESHOLDS.hostingCdnSpendSurface.highRequestCount);
        const mediumHosting = scoredHostingVendors.length >= 2 ||
            metrics.totalEncodedBodySize >= config_1.DETECTION_THRESHOLDS.hostingCdnSpendSurface.mediumTransferBytes ||
            metrics.requestCount >= config_1.DETECTION_THRESHOLDS.hostingCdnSpendSurface.mediumRequestCount;
        issues.push({
            ...
            threshold: {
                totalEncodedBodySize: highHosting
                    ? config_1.DETECTION_THRESHOLDS.hostingCdnSpendSurface.highTransferBytes
                    : mediumHosting
                        ? config_1.DETECTION_THRESHOLDS.hostingCdnSpendSurface.mediumTransferBytes
                        : config_1.DETECTION_THRESHOLDS.hostingCdnSpendSurface.lowTransferBytes
            }
        });
    }
