...
exports.buildPlusOptimizationReport = buildPlusOptimizationReport;
const config_1 = require("./config");
function countAnswered(answers) {
    return Object.values(answers).filter((value) => value !== undefined).length;
}
function getMissingCoreQuestions(answers) {
    return config_1.PLUS_CORE_KEYS.filter((key) => answers[key] === undefined);
}
function buildPriorityLabel(answers, score) {
    let priorityScore = 0;
...
function buildContextPrefix(answers) {
    const parts = [];
    if (answers.hostingProvider) {
        parts.push(`On ${config_1.PLUS_LABELS.hostingProvider[answers.hostingProvider]}`);
    }
    if (answers.monthlyVisits) {
        parts.push(config_1.PLUS_LABELS.monthlyVisits[answers.monthlyVisits]);
    }
    if (answers.appType) {
        parts.push(`for this ${config_1.PLUS_LABELS.appType[answers.appType]}`);
    }
    if (answers.siteSize) {
        parts.push(`across ${config_1.PLUS_LABELS.siteSize[answers.siteSize]}`);
    }
    return parts.length > 0 ? `${parts.join(" ")}:` : "";
}
function buildCostSensitivityNote(answers, primaryCategory) {
    const notes = [];
...
function buildProviderNextStep(answers, primaryCategory, baseNextStep) {
...
function buildPlusOptimizationReport(baseInsight, snapshot, issues, score, answers) {
    ...
    const routePriority = answers.highTrafficRoute === "yes"
        ...
        : answers.highTrafficRoute === "somewhat"
            ...
            : "";
    const planNote = answers.hostingPlan === "free"
