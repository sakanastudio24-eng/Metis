...
function classifyResourceCategory(
...
function buildTopOffenders(aggregates: ResourceAggregate[]): ResourceAggregate[] {
  return [...aggregates]
...
export function buildResourceMetrics(
  ...
  overrides: Partial<
    Pick<
      ResourceMetricsSummary,
      "rawRequestCount" | "droppedZeroTransferCount" | "droppedTinyCount"
    >
  > = {}
): ResourceMetricsSummary {
  const resourceCounts = new Map<string, number>();
  const aggregateByUrl = new Map<string, ResourceAggregate>();
  ...
  for (const resource of resources) {
    ...
    if (existingAggregate) {
      ...
      existingAggregate.largestEncodedBodySize = Math.max(
        existingAggregate.largestEncodedBodySize,
  ...
  return {
    ...
    largeAssetCount: resources.filter(
      (resource) => resource.encodedBodySize >= MEANINGFUL_IMAGE_BYTES
    ).length,
    ...
    topMeaningfulImages: buildTopOffenders(
      aggregates.filter(
        (aggregate) =>
          aggregate.category === "image" && aggregate.largestEncodedBodySize >= MEANINGFUL_IMAGE_BYTES
      )
    )
  };
}
